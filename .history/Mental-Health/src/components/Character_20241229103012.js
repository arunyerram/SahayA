...
const Character = ({ glbPath, currentAnimationName, position, texturePath }) => {
  ...
  useEffect(() => {
    if (actions[currentAnimationName]) {
      ...
      return () => {
        // Only fade out when the component unmounts
        action.fadeOut(0.5);
      };
    } else {
      console.warn(`Animation "${currentAnimationName}" not found in ${glbPath}`);
  ...
  return (
    <group ref={group} position={position} dispose={null}>
      <group name="Scene">
        {glbPath === "/stacy.glb" ? (
          <group name="Stacy" rotation={[0, 0, 0]} scale={0.01}>
            <skinnedMesh
              name="stacy"
              geometry={nodes.stacy.geometry}
              material={texture ? undefined : nodes.stacy.material}
              skeleton={nodes.stacy.skeleton}
            >
              {texture && <meshStandardMaterial map={texture} map-flipY={false} />}
            </skinnedMesh>
  ...
  return (
    <>
      ...
      <Character
        glbPath="/6758799f130072afa6b4b6da.glb"
        currentAnimationName="Idle"
        position={[-2, 0, 0]} // Adjusted Y position
      />
      {/* Second Character */}
      <Character
        glbPath="/Character2.glb"
      ...
      <Character
        glbPath="/stacy.glb"
        currentAnimationName="Dance"
        position={[0, 0, 2]} // Adjusted Y position
        texturePath="/stacy.jpg"
      />
    </>
  );
