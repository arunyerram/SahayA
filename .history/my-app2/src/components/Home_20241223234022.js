...
const ChatbotWithEmoji = () => {
  const [isWebchatOpen, setIsWebchatOpen] = useState(false); // Chatbot initially closed
  const [message, setMessage] = useState(""); // Message to send to chatbot
  const [client, setClient] = useState(null);

  // Initialize Botpress client
  useEffect(() => {
    const botClient = getClient({ clientId });
    setClient(botClient);
  }, []);

  // Handle emoji click
  const handleEmojiClick = (emoji) => {
    setMessage(emoji); // Update message with the clicked emoji
    setIsWebchatOpen(true); // Open the chatbot

    if (client) {
      // Send the emoji to the chatbot
      client.sendEvent({
        type: "proactive-trigger",
        payload: { text: emoji }, // Send emoji text
      });
    } else {
      console.error("Botpress client is not initialized.");
    }
  };
