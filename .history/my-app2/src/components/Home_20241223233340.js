...
import { motion } from "framer-motion";
import { FaSmile, FaRegSadCry, FaRobot, FaHandshake } from "react-icons/fa";
import Character from "./Character"; // Assume Character is a 3D model component

const Home = () => {
  const currentAnimation = "Idle"; // Example animation state

  const handleClick = (event) => {
    alert(`You clicked on ${event.target.title}!`);
  };
  ...
  return (
    <div>
      ...
      >
        <Canvas style={{ height: "500px", width: "1000px" }}>
          <OrbitControls enableZoom={false} />
          <ambientLight intensity={0.5} />
          <directionalLight position={[10, 10, 5]} intensity={1} />
          <Character currentAnimationName={currentAnimation} />
      ...
      >
        ...
        <motion.div
          style={{
            ...
            display: "flex",
            justifyContent: "center",
            gap: "20px", // Adjusted spacing between emojis
          }}
          initial={{ scale: 0.8 }}
          animate={{ scale: 1 }}
          transition={{ duration: 0.5 }}
