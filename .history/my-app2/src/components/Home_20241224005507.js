...
import React, { useState, useEffect } from "react";
import { FaSmile, FaRegSadCry, FaRobot, FaHandshake, FaMeh, FaAngry, FaGrinStars, FaGrinHearts } from "react-icons/fa";
import { motion } from "framer-motion";
import { Link, useNavigate } from "react-router-dom";
import { Canvas } from "@react-three/fiber";
import { OrbitControls } from "@react-three/drei";
  ...
  return (
    <>
      ...
      <div style={{ textAlign: "center", margin: "02px 0" }}>
        ...
        >
          {/* Emojis revolving in a circular path */}
          {[FaSmile, FaRegSadCry, FaRobot, FaHandshake, FaMeh, FaAngry, FaGrinStars, FaGrinHearts].map((Icon, index) => (
            <Icon
              key={index}
              style={{
                position: "absolute",
                top: "50%",
                left: "50%",
                transform: `translate(-50%, -50%) rotate(${index * 45}deg) translateX(120px)`,
                fontSize: "50px",
                cursor: "pointer",
              }}
              onClick={handleClick}
