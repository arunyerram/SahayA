...
const Nav = ({ chatbotName, setChatbotName }) => {  // Added setChatbotName to update chatbotName
  const [showDropdown, setShowDropdown] = useState(false);
  const dropdownRef = useRef(null);
  ...
  const handleLogout = async () => {
    try {
      ...
      localStorage.removeItem("user");  // Adjust based on how user data is stored in localStorage

      // Reset chatbot name to "chatbot"
      setChatbotName("chatbot");  // Update chatbotName to "chatbot"
